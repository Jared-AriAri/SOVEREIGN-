    ...
    <section id="products" className="relative py-16 md:py-24 bg-[#071A22]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <span className="inline-block px-3 py-1 text-xs font-semibold tracking-wider uppercase text-[#00A99D] bg-[#00A99D]/10 rounded-full mb-4">Product Catalog</span>
          ...
          <p className="text-[#F7F9FB]/60 max-w-2xl mx-auto">
            From homes to large facilities — atmospheric water generation systems for every scale. Powered by Watergen GENius™ technology, distributed by Awanube.
          </p>
        </div>
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-3 gap-3 mb-5">
                  ...
                  <div className="text-center p-2 bg-[#071A22]/50 rounded-lg">
                    ...
                    <div className="text-sm font-semibold text-[#00A99D]">{product.powerConsumptionW >= 1000 ? `${(product.powerConsumptionW / 1000).toFixed(1)} kW` : `${product.powerConsumptionW} W`}</div>
                  </div>
                  <div className="text-center p-2 bg-[#071A22]/50 rounded-lg">
                    <div className="text-xs text-[#F7F9FB]/40 mb-1">Tank</div>
                    <div className="text-sm font-semibold text-[#2BA84A]">{product.storageTankLiters ? `${product.storageTankLiters} L` : 'N/A'}</div>
                  </div>
                </div>
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {product.idealFor.slice(0, 3).map((use, i) => (
                      <span key={i} className="px-2 py-0.5 text-[10px] text-[#F7F9FB]/50 bg-[#F7F9FB]/5 rounded-full">{use}</span>
                    ))}
                  </div>
                </div>
...
const ProductDetailPanel: React.FC<DetailPanelProps> = ({ product, onClose, onQuote, onAssessment }) => {
  const specs = [
    ...
    { label: 'Storage Tank', value: product.storageTankLiters ? `${product.storageTankLiters} L` : 'External tank recommended' },
    ...
    <div className="fixed inset-0 z-[90] flex justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mb-6">
            {product.badge && (
              <span className="inline-block px-3 py-1 text-xs font-semibold bg-[#0077C8] text-white rounded-full mb-3">{product.badge}</span>
            )}
            ...
            <ul className="space-y-2">
              {product.benefits.map((b, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-[#F7F9FB]/65">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2BA84A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 flex-shrink-0"><polyline points="20 6 9 17 4 12"/></svg>
                  {b}
                </li>
              ))}
            </ul>
          ...
          {/* Specs Table */}
          <div className="mb-8">
            <h3 className="text-sm font-semibold text-[#F7F9FB]/80 uppercase tracking-wider mb-3">Technical Specifications</h3>
            <div className="bg-[#071A22]/50 rounded-xl overflow-hidden">
              {specs.map((spec, i) => (
                <div key={i} className={`flex justify-between items-center px-4 py-3 ${i % 2 === 0 ? 'bg-[#071A22]/30' : ''}`}>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {product.idealFor.map((use, i) => (
                <span key={i} className="px-3 py-1.5 text-sm text-[#00A99D] bg-[#00A99D]/10 border border-[#00A99D]/20 rounded-lg">{use}</span>
              ))}
            </div>
          </div>
